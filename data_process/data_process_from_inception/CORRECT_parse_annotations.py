...
def preprocessing(df, deduplicate=False):
    ...
    return df.assign(
        sen_id = lambda df: df.NotitieID.astype(str) + '_' + df.sen_id.astype(str),
        tok = lambda df: df.tok_id.astype(int),
    )
...
def main(tagset, infile, deduplicate, outfile):
    ...
    tagset = "./../../../Downloads/tagset.json"
    infile = "./data/CORRECT_process_annotated_output.pkl"
    outfile = "./data/CORRECT_parse_annotations_output.pkl"
    
    with open(tagset, 'r') as f:
        tagset = json.load(f)['tags']

    df = pd.read_pickle(infile).pipe(preprocessing).pipe(parse_df, tagset)
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--outfile', default='./draft_output.pkl')
    ...
    tagset = "./../../../Downloads/tagset.json"
    infile = "./data/CORRECT_process_annotated_output.pkl"
    outfile = "./CORRECT_parse_annotations_output.pkl"
